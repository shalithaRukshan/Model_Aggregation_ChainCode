package org.hyperledger.fabric.samples.fl;
...
@Contract(
        name = "maskedfl",
        info = @Info(
                title = "Masked Federated Learning",
                description = "Secure federated learning with masking",
...
@Default
public final class MaskedFL implements ContractInterface {

    private static final Logger logger = LogManager.getLogger(MaskedFL.class);
    ...
    public void InitLedger(final Context ctx) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ...
        ctx.getStub().putStringState("EC_Param", "brainpoolP256r1");
    ...
    public String GetECCurve(final Context ctx) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ChaincodeStub stub = ctx.getStub();
        logger.info("Getting curve details from the ledger");

        String curve = stub.getStringState("EC_Param");
        logger.info("Curve in the ledger is: " + curve);
        return curve;

    }

    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public void StoreModel(final Context ctx, String id, String model) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ChaincodeStub stub = ctx.getStub();
        logger.info("Submitting the initial points to generate system");
        int idm = Integer.parseInt(id);
        stub.putStringState("EC_model_" + idm, model);
        logger.info("Adding model successful: "+ idm);
//        String[] models = model.split("\\$\\$");
//        for (String m :
//                models) {
//            stub.putStringState("EC_model_" + idm, m);
//            logger.info("Adding model successful: "+ idm);
//            String storedmodel = stub.getStringState("EC_model" + idm);
//            System.out.println("model stored is " + storedmodel);
//            idm= idm + 1;
//        }
    ...
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public String AggregateModel(final Context ctx, String startIndex, String users, String aggModelId) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ChaincodeStub stub = ctx.getStub();
        logger.info("Getting aggregate model and updating the ledger");

        int numUsers = Integer.parseInt(users);
        int sIndex = Integer.parseInt(startIndex);
        ArrayList<String[]> models = new ArrayList<>();
        for (int i = sIndex; i < sIndex + numUsers; i++) {
            String model = stub.getStringState("EC_model_" + i);
//            logger.info("read model: " + model);
        ...
        for (int j = 0; j < firstmodel.length; j++) {
            final int m = j;
            List<Long> elements = models.stream()
                    .map(x -> Long.valueOf(x[m].toString())) // you might need toString() here if your array is an Object[]
                    .collect(Collectors.toList());
            Long sum = elements.stream()
                    .reduce(Long.parseLong("0"), Long::sum);
            aggmodel = aggmodel + "||" + (long) (sum / numUsers);

        }
        stub.putStringState(aggModelId, aggmodel);
//        logger.info("aggregated model is: " + aggmodel);
        return aggmodel;

    }

    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetAggModel(final Context ctx, String modelId) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ChaincodeStub stub = ctx.getStub();
        logger.info("Getting model details from the ledger");

        String model = stub.getStringState(modelId);
        logger.info("model aggregated is: " + model);
        return model;

    }

    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetModel(final Context ctx, String modelId) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidAlgorithmParameterException {
        ChaincodeStub stub = ctx.getStub();
        logger.info("Getting model details from the ledger");

        String model = stub.getStringState("EC_model_" + modelId);
        logger.info("model for getting is: " + model);
        return model;
    ...
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public String NFCertRequest(final Context ctx, final String request) throws NoSuchAlgorithmException,
            NoSuchProviderException, InvalidKeySpecException, InvalidAlgorithmParameterException,
